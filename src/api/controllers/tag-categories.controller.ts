...
export function createTagCategoriesController(service: TagCategoriesService, tagsService: TagsService): Record<string, RequestHandler> {
  return {
    ...
    delete: (request, response, next) => {
      void tagsService.deleteCategoryWithTags(userId(request), categoryId(request)).then(() => response.status(204).end()).catch(next);
    },
