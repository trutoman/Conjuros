...
export function ItemForm({
  ...
  return <form className="item-form" onSubmit={submit}><h2>{item ? 'Edit item' : 'Add item'}</h2><ItemTypeSelector value={kind} onChange={setKind} /><FormField label="Title"><input value={title} onChange={(event) => setTitle(event.target.value)} /></FormField><FormField label="Description"><textarea value={description} onChange={(event) => setDescription(event.target.value)} /></FormField><FormField label={kind === 'spell' ? 'Command' : 'URL'} error={error}><textarea value={content} onChange={(event) => setContent(event.target.value)} /></FormField><fieldset className="item-form-tags"><legend>Tags</legend>{availableTags.map((tag) => { const normalized = tag.tagName.toLowerCase(); const checked = tags.includes(normalized); return <label key={tag.id} className="tag-filter-pill" style={{ color: tag.color, borderColor: tag.color, background: `color-mix(in srgb, ${tag.color} ${checked ? 20 : 8}%, var(--surface))` }}><input type="checkbox" aria-label={tag.tagName} checked={checked} onChange={(event) => { const next = event.target.checked ? [...tags, normalized] : tags.filter((candidate) => candidate !== normalized); setTags([...new Set(next)]); }} />{tag.tagName}</label>; })}</fieldset><div className="form-actions"><button type="submit">Save item</button><button type="button" className="quiet" onClick={onCancel}>Cancel</button></div></form>;
}