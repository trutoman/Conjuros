...
export function CollectionPage({ onSignOut }: { onSignOut?: () => void }) {
  ...
  const visibleItems = items.filter((item) => {
    ...
    return (!filters.search || searchable.includes(filters.search.toLowerCase())) &&
      ...
      (filters.tags.length === 0 || (filters.tagFilterMode === 'any'
  ...
  {deleteItem && <DeleteConfirmDialog title={deleteItem.title} onConfirm={() => void confirmDelete()} onCancel={() => setDeleteItem(null)} />}
  {deleteTag && <DeleteConfirmDialog title={deleteTag.tagName} onConfirm={() => void confirmTagDelete()} onCancel={() => setDeleteTag(null)} />}
  </main>;
}